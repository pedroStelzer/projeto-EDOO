...
#include "ui_horariosprofessor.h"

// Construtor da classe Horariosprofessor
Horariosprofessor::Horariosprofessor(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::Horariosprofessor)
{
    ui->setupUi(this);
    ...
    botaoConfirmar->setFixedSize(100, 20); // Largura

    // Conectar o botão Confirmar a uma função que será chamada quando o botão for pressionado
    connect(botaoConfirmar, &QPushButton::clicked, this, &Horariosprofessor::onConfirmarClicked);
...
Horariosprofessor::~Horariosprofessor()
{
    delete ui;
}

// Método para obter os horários selecionados
QMap<QString, QStringList> Horariosprofessor::getHorariosSelecionados()
{
    return horariosSelecionados;
};

// Função chamada quando o botão Confirmar é pressionado
void Horariosprofessor::onConfirmarClicked()
{
